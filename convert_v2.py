...
def folder_create():
    ...
    if isExist is True:
        ...
        for items in list:
            ...
            os.mkdir(path)
#        if isExist is True:
#            fldr_check_var.set("Folder created")
#        else:
#            fldr_check_var.set("Folder still does not exist")
...
lbl = Label(window, text="Вставьте название сделки:")
...
lbl_2 = Label(window, text="Вставьте название папки:")
...
btn_folder_check.grid(column=1, row=3)
lbl_fldr_check.grid(column=1, row=4)
btn_folder_create.grid(column=1, row=5)
txt_in.focus()
window.mainloop()
